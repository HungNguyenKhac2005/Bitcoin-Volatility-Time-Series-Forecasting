...
from tft import LiteTFTModel

# ========================================================= #
# QUÁ TRÌNH INFERENCE: DỰ ĐOÁN, GIẢI MÃ SCALER VÀ ĐÁNH GIÁ
# ========================================================= #
def predict_TFT_real():
    # 1. Cấu hình thiết bị (GPU/CPU)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"🚀 Đang khởi chạy quá trình dự đoán Lite TFT trên thiết bị: {device}")
    ...
    # Trỏ đúng đến file trọng số của mô hình TFT
    MODEL_PATH = os.path.join(BASE_DIR, 'src', 'model_save', 'tft_best.pt')
    ...
    # =========================================================
    print("⏳ Đang nạp trọng số (weights) của mô hình Lite TFT tốt nhất...")
    # Khởi tạo kiến trúc mô hình (input_dim=8)
    ...
    # Chuyển mô hình sang chế độ suy luận (Tắt Dropout)
    model.eval()
    ...
    with torch.no_grad(): # Tắt tính toán Gradient để tối ưu tốc độ và bộ nhớ
    ...
    # Lưu trữ mảng dự đoán ra file .pkl phục vụ hệ thống Stacking Meta-Learner sau này
    os.makedirs(MODELS_DIR, exist_ok=True)
    pkl_save_path = os.path.join(MODELS_DIR, 'preds_real_tft.pkl')
    ...
    # Sử dụng màu 'orange' thay vì 'yellow' để hiển thị rõ hơn trên nền trắng
    plt.plot(test_set_transformer_timestamp['timestamp'], preds_real, color='darkorange', label='Lite TFT Predicted', linewidth=1.5)
    ...
    # Định dạng trục X (Timestamp) để chống lộn xộn
    ticks = np.linspace(0, len(test_set_transformer_timestamp['timestamp']) - 1, 10, dtype=int)
    plt.xticks(ticks, rotation=45)
    
    plt.title("So sánh Độ biến động (Volatility): Dự đoán bởi Lite TFT vs Thực tế")
    ...
    print("\n" + "="*50)
    print("📊 KẾT QUẢ ĐÁNH GIÁ MÔ HÌNH LITE TFT (HỆ THỰC TẾ)")
    print("="*50)
