...
# --- IMPORT KIẾN TRÚC MÔ HÌNH VÀ THAM SỐ ---
from transformer_model import TimeseriesTransformer, get_args

# ========================================================= #
# QUÁ TRÌNH INFERENCE: DỰ ĐOÁN, GIẢI MÃ SCALER VÀ ĐÁNH GIÁ
# ========================================================= #
def predict_transformer_real():
    # 1. Cấu hình thiết bị (Tối ưu hóa chạy trên GPU nếu có)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"🚀 Đang khởi chạy quá trình dự đoán Transformer trên thiết bị: {device}")
    ...
    # Trỏ đúng đến file trọng số của mô hình Transformer
    ...
    # =========================================================
    print("⏳ Đang tải trọng số (weights) của mô hình Transformer tốt nhất...")
    # Lấy siêu tham số cấu trúc (Hyperparameters)
    args = get_args()
    
    # Khởi tạo kiến trúc mô hình Transformer với các tham số từ args
    ...
    # Chuyển sang chế độ suy luận: Khóa LayerNorm và tắt Dropout
    model.eval()

    print("🧠 Đang xử lý tính toán (Forward Pass) thông qua cơ chế Self-Attention trên tập Test...")
    ...
    # Lưu trữ mảng dự đoán ra file .pkl phục vụ hệ thống Stacking Ensemble (XGBoost)
    os.makedirs(MODELS_DIR, exist_ok=True)
    pkl_save_path = os.path.join(MODELS_DIR, 'preds_real_transformer.pkl')
    ...
    # Sử dụng màu đỏ để làm nổi bật kết quả của mô hình lõi (Transformer)
    plt.plot(test_set_transformer_timestamp['timestamp'], preds_real, color='red', label='Transformer Predicted', linewidth=1.5)
    ...
    # Tối ưu hóa trục X (Timestamp) để không bị lộn xộn
    ticks = np.linspace(0, len(test_set_transformer_timestamp['timestamp']) - 1, 10, dtype=int)
    plt.xticks(ticks, rotation=45)
    
    plt.title("So sánh Độ biến động (Volatility): Dự đoán bởi Transformer vs Thực tế")
    plt.xlabel("Thời gian (Timestamp)")
    plt.ylabel("Độ biến động (Annualized Volatility)")
    plt.legend()
    plt.tight_layout() # Đảm bảo các thành phần không bị cắt xén
    plt.show()

    # Báo cáo kết quả trên Console
    print("\n" + "="*50)
    print("📊 KẾT QUẢ ĐÁNH GIÁ MÔ HÌNH TRANSFORMER (HỆ THỰC TẾ)")
    print("="*50)
