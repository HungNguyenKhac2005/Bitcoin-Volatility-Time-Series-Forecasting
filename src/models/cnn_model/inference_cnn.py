...
# ========================================================= #
def predict_CNN_real():
    # 1. Cấu hình thiết bị
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"🚀 Đang khởi chạy quá trình dự đoán CNN 1D trên thiết bị: {device}")
    ...
    MODEL_PATH = os.path.join(BASE_DIR, 'src', 'model_save', 'cnn_best.pt')
    ...
    if not os.path.exists(test_set_transformer_path):
        print(f"❌ Cảnh báo: Không tìm thấy file {test_set_transformer_path}. Vui lòng chạy luồng tiền xử lý (Preprocessing) trước.")
        return
    ...
    # =========================================================
    print("⏳ Đang nạp trọng số (weights) tốt nhất của CNN 1D...")
    model = CNN1DModel(input_dim=8).to(device) 
    
    if os.path.exists(MODEL_PATH):
        # Nạp state_dict một cách an toàn (weights_only=True)
    ...
    model.eval() # Chuyển mô hình sang chế độ suy luận (tắt Dropout, BatchNorm tĩnh)
    ...
    with torch.no_grad(): # Tắt lưu trữ Gradient để tiết kiệm bộ nhớ
    ...
    if os.path.exists(SCALER_PATH):
        ...
        # Gắn kết quả dự đoán vào đúng cột mục tiêu (cột số 8)
        TARGET_COL_INDEX = 8
        dummy_array[:, TARGET_COL_INDEX] = preds_scaled
        
        # Inverse transform và trích xuất lại cột mục tiêu
    ...
    # Lưu trữ mảng dự đoán thực tế ra file .pkl
    os.makedirs(MODELS_DIR, exist_ok=True)
    pkl_save_path = os.path.join(MODELS_DIR, 'preds_real_cnn.pkl')
    ...
    plt.plot(test_set_transformer_timestamp['timestamp'], preds_real, color='brown', label='CNN 1D Predicted', linewidth=1.5)
    ...
    # Tối ưu hóa hiển thị trục X
    ticks = np.linspace(0, len(test_set_transformer_timestamp['timestamp']) - 1, 10, dtype=int)
    plt.xticks(ticks, rotation=45)
    
    plt.title("So sánh Độ biến động (Volatility): Dự đoán bởi CNN 1D vs Thực tế")
    ...
    # Báo cáo kết quả Console
    print("\n" + "="*50)
    print("📊 KẾT QUẢ ĐÁNH GIÁ MÔ HÌNH CNN 1D (HỆ THỰC TẾ)")
    print("="*50)
